...
export function AuthScreenLayout({
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-background px-4 py-10 sm:px-6">
      ...
      <div
        aria-hidden="true"
        className="auth-bg-gradient absolute inset-0 opacity-70 [contain:layout_style_paint]"
      />
      <div
        aria-hidden="true"
        className="absolute inset-0 opacity-30 [contain:layout_style_paint] [background-image:linear-gradient(to_right,hsl(var(--border)/0.22)_1px,transparent_1px),linear-gradient(to_bottom,hsl(var(--border)/0.22)_1px,transparent_1px)] [background-size:72px_72px] [mask-image:radial-gradient(circle_at_center,black_26%,transparent_82%)]"
      />
      ...
      <main id="auth-content" className="relative mx-auto flex min-h-[calc(100vh-5rem)] w-full max-w-md items-start justify-center pt-[12vh]">
        <div className="w-full">
          <div className="mb-8 text-center">
            ...
            <div aria-hidden="true" className="mb-3 inline-flex items-center rounded-full border border-border/60 bg-card/60 px-3 py-1 text-xs font-medium uppercase tracking-[0.24em] text-muted-foreground">
              {badge}
            </div>
