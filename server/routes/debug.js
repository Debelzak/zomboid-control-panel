...
router.post('/clear-stale-locks', async (req, res) => {
  try {
    ...
    } catch { running = !!serverManager?.isRunning; }
    if (running) {
      return res.status(409).json({
        success: false,
        error: 'Stop the server before clearing lock files. PZ holds these open while running.'
      });
    }

    const activeServer = await getActiveServer().catch(() => null);
    if (!activeServer) {
      return res.status(400).json({ success: false, error: 'No active server is configured.' });
    }
    const zPath = activeServer.zomboidDataPath || null;
    if (!zPath || !activeServer.serverName) {
      return res.status(400).json({
        success: false,
        error: 'Active server has no Zomboid data path or server name configured.'
      });
    }

    const savesRoot = path.join(zPath, 'Saves');
    const candidates = [path.join(savesRoot, 'Multiplayer', activeServer.serverName)];
    if (activeServer.savename && activeServer.savename !== activeServer.serverName) {
      candidates.push(path.join(savesRoot, 'Multiplayer', activeServer.savename));
    }
    let saveDir = null;
    for (const sp of candidates) {
      try {
        const st = await fs.promises.stat(sp);
        if (st.isDirectory()) { saveDir = sp; break; }
      } catch { /* not present */ }
    }
    if (!saveDir) {
      return res.status(404).json({ success: false, error: 'Active save folder not found.' });
    }
    ...
    const walk = async (dir) => {
      ...
      for (const name of names) {
        ...
        if (st.isDirectory()) {
          await walk(full);
        } else if (st.isFile() && (name.endsWith('.lock') || name === '.lock')) {
    ...
    log.info(`Cleared ${deleted.length} stale lock file(s) from ${saveDir} (${failed.length} failed)`);
    res.json({
      success: true,
      ...
      truncated,
      saveDir,
      message: `Removed ${deleted.length} stale lock file${deleted.length === 1 ? '' : 's'}` +
