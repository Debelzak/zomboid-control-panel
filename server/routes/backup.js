...
router.post(
  ...
  async (req, res) => {
    try {
      const activeServer = await getActiveServer();
      if (activeServer?.isRemote) {
        return res.status(400).json({ error: 'Backup upload is not available for remote servers.' });
      }

      if (!req.body || !Buffer.isBuffer(req.body) || req.body.length === 0) {
        return res.status(400).json({ error: 'No file uploaded. Send the zip body with Content-Type: application/zip.' });
      }
      ...
      if (req.body.length < 4 || req.body[0] !== 0x50 || req.body[1] !== 0x4B) {
        return res.status(400).json({ error: 'File does not look like a valid .zip archive.' });
      }

      const rawName = String(req.headers['x-backup-filename'] || 'uploaded-backup.zip');
      ...
      const backupsPath = await backupService.getBackupsPath();
      if (!backupsPath) {
        return res.status(500).json({ error: 'Backups folder not available. Configure the server first.' });
      }
      ...
      if (fs.existsSync(targetPath)) {
        return res.status(409).json({ error: `A backup named "${finalName}" already exists. Delete it first or rename the upload.` });
      }
