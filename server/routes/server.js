...
router.post('/wipe/preview', async (req, res) => {
  try {
    ...
    if (!Array.isArray(targets) || targets.length === 0) {
      return res.status(400).json({ error: 'targets must be a non-empty array of: map, players, world' });
    }
    ...
    const MAP_DIRS = ['map', 'chunkdata', 'isoregiondata', 'zpop', 'apop', 'metagrid'];
    const WORLD_DIRS = ['radio'];
    // Player files in save root
    const PLAYER_ROOT_FILES = /^(players\.db|players\.db-journal|vehicles\.db|vehicles\.db-journal|map_p\.bin|map_zone\.bin)$/i;
    ...
    const WORLD_ROOT_FILES = /^(WorldDictionary.*|map_meta\.bin|map_t\.bin|map_worldgen\.bin|map_animals\.bin|map_basements\.bin|entity_data\.bin|global_mod_data\.bin|reanimated\.bin|iTrack\.bin|gos_.*\.bin|id_manager_data\.bin|important_area_data\.bin|z_outfits\.bin|recorded_media\.bin|servermap_symbols\.bin|map_sand\.bin|erosion\.ini)$/i;
      ...
      try {
        ...
        for (const entry of rootEntries) {
          if (!entry.isDirectory() && WORLD_ROOT_FILES.test(entry.name)) {
            worldFiles++;
            try { worldSize += fs.statSync(path.join(saveDir, entry.name)).size; } catch (e) {
              log.debug(`Stat failed for world file ${entry.name}: ${e.message}`);
            }
          }
        }
      } catch (e) {
  ...
  try {
    ...
    try {
      if (targets.includes('map')) {
        ...
        results.map = deletedCount > 0 ? `deleted ${deletedCount} directories` : 'not found';
      }
      ...
      if (targets.includes('players')) {
        ...
        results.players = deletedCount > 0 ? `deleted ${deletedCount} files` : 'not found';
      }
      ...
      if (targets.includes('world')) {
        ...
        results.world = deletedCount > 0 ? `deleted ${deletedCount} items` : 'not found';
      }
    } finally {
