...
function getSteamCmdExe(steamcmdPath) {
  return path.join(steamcmdPath, isWindows ? 'steamcmd.exe' : 'steamcmd.sh');
}
  ...
  try {
    ...
    if (!fs.existsSync(iniPath)) {
      ...
      try {
        ...
        fs.writeFileSync(iniPath, minimalIni, 'utf-8');
    ...
    steamcmd.on('close', async (code) => {
      ...
      if (code === 0) {
        ...
        try {
          ...
          fs.writeFileSync(shellPath, scripts.sh, { encoding: 'utf8', mode: 0o755 });
...
router.post('/steamcmd/download', async (req, res) => {
  try {
    const defaultPath = isWindows ? 'C:\\SteamCMD' : path.join(process.env.HOME || '/opt', 'steamcmd');
